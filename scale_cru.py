    ...
    for x in range(i,i+n):
        for y in range(j,j+n):
            ...
            if grid_id in grid_list:
                grid_id='%03d%03d' % (x,y)
                df = pd.read_csv(r'F:\crop-climate\crucsv/%s.csv' % (grid_id))
                presum+=df['Pre']*area(x,y)
                tmpsum+=df['Tmp']*area(x,y)
                areasum+=area(x,y)
    if areasum!=0:
        presum=presum/areasum
        tmpsum=tmpsum/areasum
        dataframe=pd.DataFrame({'Year':df['Year']})
        dataframe['Pre']=presum
        dataframe['Tmp']=tmpsum
        grid_id='%03d%03d' % (i,j)
        dataframe.to_csv(r'F:\crop-climate\cru_scale\360/%s.csv' % (grid_id),index=False)

if __name__ == '__main__':
    df = pd.read_csv(r'F:\crop-climate\cru_scale\grid_id.csv',index_col=False)
